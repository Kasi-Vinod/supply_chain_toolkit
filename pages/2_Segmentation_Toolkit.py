...
# ---------- PDF Generator ----------
def create_summary_pdf(summary_df, inputs_dict, preview_df, figs):
    buffer = BytesIO()
    ...
    # Results (Preview)
    story.append(Paragraph("Results (First 20 rows)", styles["Heading2"]))
    head = preview_df.head(20)
    res_table = [head.columns.to_list()] + head.values.tolist()
    table3 = Table(res_table)
...
if uploaded_file:
    ...
    if st.button("Run Product Segmentation"):
        ...
        pdf_buf = create_summary_pdf(summary, inputs_dict, result, figs)
